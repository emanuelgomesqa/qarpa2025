...
When('eu esteja usando o dispositivo {string}', (resolucao) => {
  cy.viewport(`${resolucao}`)
});

And('verifico que não existe produtos um ao lado do outro', ()=>{
  cy.get('.inventory_list')
      .should('be.visible')
      .then(($list) => {
        const lisWidth = $list.width()

        cy.get('.inventory_item').each(($item) => {
          expect($item.width()).to.be.closeTo(lisWidth, 25)
        })
      })
})
